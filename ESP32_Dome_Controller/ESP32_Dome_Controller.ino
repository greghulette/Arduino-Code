...
void setup(){
  ...
  colorWipe(red, 6); // red during bootup
      ...
      if( inputBuffer[0]=='D' ||        // Door Designator
          ...
          if(commandLength >= 3) {
            ...
            if(inputBuffer[0]=='R' || inputBuffer[0]=='r') {
              ...
              printf("Buffer 0:%c 1:%c 2:%c 3:%c 4:%c \n", inputBuffer[0], inputBuffer[1], inputBuffer[2], inputBuffer[3], inputBuffer[4]);
              printf("Function: %d , Color: %d, Speed: %d \n", ledFunction, colorState1, speedState);
              }              
            ...
            if(inputBuffer[0]=='R' || inputBuffer[0]=='r'){
              ...
              printf("CL_Command 0: %d, 1: %d, 2:%d \n", CL_command[0], CL_command[1], CL_command[2]);
//              CLMillis = millis();
            }
...
void panelWave(int servoBoard, int servoEasingMethod, uint32_t servoMovementDuration){
  ...
  switch(servoBoard){
    ...
    case 4: SEQUENCE_PLAY_ONCE_SPEED(servoSequencer, SeqPanelWave, ALL_SERVOS_MASK, servoMovementDuration); break;
            DelayCall::schedule([stringToSend]{sendESPNOWCommand("BS", stringToSend);}, 2000); break;
  }
  D_command[0]   = '\0';                                             
}
