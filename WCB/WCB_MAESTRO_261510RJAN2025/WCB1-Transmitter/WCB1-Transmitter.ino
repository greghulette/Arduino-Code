...
// Peer MAC address (replace with Receiver ESP32 MAC)
uint8_t peerMAC[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

// Circular buffer for Serial1 data
...
void onDataReceive(const esp_now_recv_info *info, const uint8_t *data, int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    Serial1.write(data[i]);  // Forward to Serial1
    Serial2.write(data[i]);  // Forward to Serial2
  }
  Serial.println();
}

// Task to handle Serial1 data
void handleSerial(void *pvParameters) {
  for (;;) {
    if (Serial1.available()) {
      uint8_t byte = Serial1.read();
      enqueueByte(byte);
      Serial2.write(byte);  // Forward to Serial2 immediately
    }
