...
void processESPNOWIncomingMessage() {
  ...
  if (incomingTargetID == "BS" || incomingTargetID == "BR") {
    ...
    stringComplete = true;
    Debug.ESPNOW("Received command from %s\n", incomingSenderID.c_str());
  }
}
//////////////////////////////////////////////////////////////////////
...
void replyServoLimits(uint8_t start, uint8_t count) {
  ...
  char hdr[10];
  snprintf(hdr, sizeof(hdr), "#LVBS%02u%02u", start, count);
  String reply = hdr;
  servoPrefs.begin("srvlim", true);
...
// its display name. Reply: "#LIBS<idx2><count2><close4><open4><name>". The web
...
void replyServoInfo(uint8_t idx) {
  ...
  char hdr[20];
  snprintf(hdr, sizeof(hdr), "#LIBS%02u%02u%04u%04u", idx, total, cl, op);
  String reply = hdr;
            ...
            if (inputBuffer[1]=='D' || inputBuffer[1]=='d'){
              ...
              debugInputIdentifier = "";                             // flush the string
                ...
                if (lSub == 'O' || lSub == 'C' || lSub == 'R' || lSub == 'L' || lSub == 'M' || lSub == 'Z' || lSub == 'G' || lSub == 'D') {
                  // Servo limit command: #LMxx / #LOxx / #LCxx / #LRxx / #LL / #LZ
