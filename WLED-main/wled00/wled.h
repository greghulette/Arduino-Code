...
// Optional dedicated UART for the Adalight/TPM2/serial JSON protocol (ESP32 only).
// Define WLED_SERIAL_RX_PIN and WLED_SERIAL_TX_PIN to move the protocol onto its own
// port; the default port then stays free as the debug/Improv console.
// WLED_SERIAL_PORT selects which UART object is used (Serial1 unless overridden).
#if defined(ARDUINO_ARCH_ESP32) && defined(WLED_SERIAL_RX_PIN) && defined(WLED_SERIAL_TX_PIN)
  #define WLED_DEDICATED_SERIAL
  #ifndef WLED_SERIAL_PORT
    #define WLED_SERIAL_PORT Serial1
  #endif
  #define WLEDSerial WLED_SERIAL_PORT
  constexpr int8_t protocolRX = WLED_SERIAL_RX_PIN;
  constexpr int8_t protocolTX = WLED_SERIAL_TX_PIN;
  // the pins are reserved in WLED::setup(), so the port is always usable
  #define protocolCanRX true
  #define protocolCanTX true
#else
  #define WLEDSerial Serial
  // the protocol shares the console, so it is only usable while those pins are free
  #define protocolCanRX serialCanRX
  #define protocolCanTX serialCanTX
#endif
