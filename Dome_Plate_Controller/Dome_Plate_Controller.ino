...
#include "esp_wifi.h"
#include <esp_now.h>
#include <Preferences.h>   // NVS storage for per-servo open/close calibration
...
// -------------------------------------------------------------------------------
// Servo Limit NVS Management  (mirrors the Dome/Body Servo controllers)
// Commands (reach over the mesh as ":EDP#LM..." etc.):
//   #LMxxyyyy  LIVE move servo xx to yyyy µs (no save) — for slider calibration
//   #LOxxyyyy  set OPEN  (end)   limit for servo xx to yyyy µs   e.g. #LO011300
//   #LCxxyyyy  set CLOSE (start) limit for servo xx to yyyy µs   e.g. #LC011500
//   #LRxx      reset servo xx to compiled-in defaults
//   #LL        list all current servo limits
//   #LZ        reload all limits from NVS (restore after live calibration)
// -------------------------------------------------------------------------------
...
void loadServoLimits() {
  ...
  for (uint8_t i = 0; i < SizeOfArray(servoSettings); i++) {
    ...
    // No NVS value -> restore the compiled default (also fixes a prior live-move widen).
...
void setServoOpenLimit(uint8_t num, uint16_t pulse) {
  ...
  servoPrefs.end();
  Serial.printf("[SERVO] Servo %02d open  = %u us (saved)\n", num, pulse);
}
...
void setServoCloseLimit(uint8_t num, uint16_t pulse) {
  ...
  servoPrefs.putUShort(key, pulse);
  servoPrefs.end();
  Serial.printf("[SERVO] Servo %02d close = %u us (saved)\n", num, pulse);
}
...
// [min,max] of the current start/end) can reach any pulse 500-2500. Real limits
// get restored as you set each end (#LO/#LC) or via #LZ (reload all from NVS).
...
void replyServoLimits(uint8_t start, uint8_t count) {
  ...
  char hdr[10];
  snprintf(hdr, sizeof(hdr), "#LVDP%02u%02u", start, count);
  String reply = hdr;
  servoPrefs.begin("srvlim", true);
...
// its display name. Reply: "#LIDP<idx2><count2><close4><open4><name>". The web
...
void replyServoInfo(uint8_t idx) {
  ...
  char hdr[20];
  snprintf(hdr, sizeof(hdr), "#LIDP%02u%02u%04u%04u", idx, total, cl, op);
  String reply = hdr;
...
void setup(){
  ...
  SetupEvent::ready();
  loadServoLimits();   // restore any NVS servo open/close overrides
